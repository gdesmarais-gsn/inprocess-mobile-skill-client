...
namespace GSN.Skill.Utils
{
	...
	{
		private const string URL_TAG_AND_CONTENTS_PATTERN = @"<a\b[^>]*>(.*?)</a>";
		private const string URL_TAG_PATTERN = @"<[^>]*>";
        ...
        public static string ReplaceRegisterTradmark(string str)
        {
            string formattedStr = str;
            if (str.Contains("&reg;"))
            {
                formattedStr = str.Replace("&reg;", "®"); //maybe swap to unicode
            }
            return formattedStr;
        }
		...
		public static string StripCarriageReturn(this string str)
		{
			...
			return Regex.Replace(str, findCarriageReturnRegExp, string.Empty);
		}

		public static string StripHtmlUrl(this string str)
		{
			return Regex.Replace(str, URL_TAG_AND_CONTENTS_PATTERN, string.Empty);
		}
		public static string StripHTMLTags(this string str)
		{
			return Regex.Replace(str, URL_TAG_PATTERN, string.Empty);
		}
		...
		public static int CompareWithNumericSearch(this string s1, string s2, bool ignoreCase = false)
		{
			...
			int minSubstringsCount = Math.Min(s1SubstringsCount, s2SubstringsCount);
			for (uint currentSubstring = 0; currentSubstring < minSubstringsCount; currentSubstring++, currentIsDigit = !currentIsDigit)
			{
