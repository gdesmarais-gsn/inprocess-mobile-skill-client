...
Shader "Projector/Intensify Masked onto Alpha" {
   Properties {
...
half4 frag (v2f i) : COLOR
{
	...
	//return float4(frac(i.uv.x / i.uv.z), frac(i.uv.y / i.uv.w), 0,0);
	fixed4 targ = tex2D(_TargetTex, i.uvo);
	//tex *= targ.g + targ.g; // RGBB
	tex *= targ.a; // alpha
	//tex.rgb += targ.a * float3(1.5,0.25,0.125);
	//tex.r += tex.r - (targ.a - 0.5) * (targ.a - 0.5);
	tex.rg += tex.r * targ.a * fixed2(0.75,0.075);
	//tex *= targ.g + targ.g; // RGBB
