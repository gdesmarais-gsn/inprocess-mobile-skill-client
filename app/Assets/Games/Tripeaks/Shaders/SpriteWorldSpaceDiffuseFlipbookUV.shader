Shader "2d/WorldSpace Sprite Diffuse FlipbookUV" {
	Properties {
		_Color ("Main Color", Color) = (1,1,1,1)
		_MainTex ("Base (RGBA)", 2D) = "white" {}
		_Speed ("1/Xcount, -1/Ycount, 1/Framerate, Xcount", Vector) = (0.2,-0.2,0.0555556,5)
	}
	SubShader {
...
CGPROGRAM
#pragma multi_compile INTERPOLATE_ON INTERPOLATE_OFF
#pragma vertex vert
...
float4 _WrappedTime;
			v2f vert (vertIn v)
			{
				...
				float4 speed = _Speed; speed.w *= speed.z;
#if INTERPOLATE_ON
				o.uv.xyzw = v.texcoord.xyxy;
				float4 speedxy = speed.xyxy;
				float4 speedzw = speed.zwzw;
				float4 time, f;
#else
				o.uv.xy = v.texcoord.xy;
				float2 speedxy = speed.xy;
				float2 speedzw = speed.zw;
				float2 time, f;
#endif
				time = _WrappedTime.w;
#if INTERPOLATE_ON
				time.zw += min(speed.z, speed.w);
#endif
				time /= speedzw;
				f = frac(time);
				time -= f;
				o.uv += frac(time * speedxy);
#if INTERPOLATE_ON
				o.f = f.x;
#endif
				return o;
...
uniform fixed4 _Color;
			float4 frag (v2f i) : COLOR
			{
				...
				fixed4 col = lerp(col1, col2, i.f);
#else
				fixed4 col = col1;
#endif
				float4 rv = _Color * col;
				//float4 rv = col;
