...
public class Reporter : MonoBehaviour {
	...
	private const string REPORTER_NAME = "Reporter";
	public static void createReporterObject()
	{
		if (GameObject.Find (REPORTER_NAME))
			return;
		GameObject reporterObj = new GameObject();
		reporterObj.name = REPORTER_NAME;
		...
		reporter.images.clearImage 			= (Texture2D)Resources.Load("Reporter/Images/clear", typeof(Texture2D));
		reporter.images.collapseImage 		= (Texture2D)Resources.Load("Reporter/Images/collapse", typeof(Texture2D));
		reporter.images.clearOnNewSceneImage= (Texture2D)Resources.Load("Reporter/Images/clearOnSceneLoaded", typeof(Texture2D));
		reporter.images.showTimeImage 		= (Texture2D)Resources.Load("Reporter/Images/timer_1", typeof(Texture2D));
		reporter.images.showSceneImage 		= (Texture2D)Resources.Load("Reporter/Images/UnityIcon", typeof(Texture2D));
		reporter.images.userImage 			= (Texture2D)Resources.Load("Reporter/Images/user", typeof(Texture2D));
		reporter.images.showMemoryImage 	= (Texture2D)Resources.Load("Reporter/Images/memory", typeof(Texture2D));
		reporter.images.softwareImage 		= (Texture2D)Resources.Load("Reporter/Images/software", typeof(Texture2D));
		reporter.images.dateImage 			= (Texture2D)Resources.Load("Reporter/Images/date", typeof(Texture2D));
		reporter.images.showFpsImage 		= (Texture2D)Resources.Load("Reporter/Images/fps", typeof(Texture2D));
		reporter.images.showGraphImage 		= (Texture2D)Resources.Load("Reporter/Images/", typeof(Texture2D));
		reporter.images.graphImage 			= (Texture2D)Resources.Load("Reporter/Images/chart", typeof(Texture2D));
		reporter.images.infoImage 			= (Texture2D)Resources.Load("Reporter/Images/info", typeof(Texture2D));
		reporter.images.searchImage 		= (Texture2D)Resources.Load("Reporter/Images/search", typeof(Texture2D));
		reporter.images.closeImage 			= (Texture2D)Resources.Load("Reporter/Images/close", typeof(Texture2D));
		reporter.images.buildFromImage 		= (Texture2D)Resources.Load("Reporter/Images/buildFrom", typeof(Texture2D));
		reporter.images.systemInfoImage 	= (Texture2D)Resources.Load("Reporter/Images/ComputerIcon", typeof(Texture2D));
		reporter.images.graphicsInfoImage 	= (Texture2D)Resources.Load("Reporter/Images/graphicCard", typeof(Texture2D));
		reporter.images.backImage 			= (Texture2D)Resources.Load("Reporter/Images/back", typeof(Texture2D));
		reporter.images.cameraImage 		= (Texture2D)Resources.Load("Reporter/Images/", typeof(Texture2D));
		reporter.images.logImage 			= (Texture2D)Resources.Load("Reporter/Images/log_icon", typeof(Texture2D));
		reporter.images.warningImage 		= (Texture2D)Resources.Load("Reporter/Images/warning_icon", typeof(Texture2D));
		reporter.images.errorImage 			= (Texture2D)Resources.Load("Reporter/Images/error_icon", typeof(Texture2D));
		reporter.images.barImage 			= (Texture2D)Resources.Load("Reporter/Images/bar", typeof(Texture2D));
		reporter.images.button_activeImage 	= (Texture2D)Resources.Load("Reporter/Images/button_active", typeof(Texture2D));
		reporter.images.even_logImage 		= (Texture2D)Resources.Load("Reporter/Images/even_log", typeof(Texture2D));
		reporter.images.odd_logImage 		= (Texture2D)Resources.Load("Reporter/Images/odd_log", typeof(Texture2D));
		reporter.images.selectedImage 		= (Texture2D)Resources.Load("Reporter/Images/selected", typeof(Texture2D));
		reporter.images.saveImage			= (Texture2D)Resources.Load("Reporter/Images/save", typeof(Texture2D));
		
		// BEGIN GSN EDIT
		reporter.images.phoenixImage		= (Texture2D)Resources.Load("WW_App_Icon", typeof(Texture2D));
		// END GSN EDIT

		reporter.images.reporterScrollerSkin = (GUISkin)Resources.Load("Reporter/Images/reporterScrollerSkin", typeof(GUISkin));
	...
	public void Initialize()
	{
		...
		saveContent = new GUIContent("", images.saveImage, "Save"); 

		// BEGIN GSN EDIT
		phoenixContent = new GUIContent("", images.phoenixImage, "Phoenix");
		// END GSN EDIT
	...
	void Start () {
		...
		// GSN EDIT:
		// 		This tries to read from build_info.txt, which doesn't make its way
		//    to the output (I'm not sure why, the code in ReporterEditor.cs might
		//    not work with our version of Unity). 
		//    Since it's not there, this shows an error.  We don't care about it, since
		//    we manage the build information on our
		// 
	...
	void DrawGSN()
	{
		...
		ReporterGSNUtilities.DrawGSNGUI();
	...
	void drawToolBar(){
		...
		if (GUILayout.Button(saveContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
		{
			try
			{
				...
				string fileName = "SparcadeLog." + now.ToString().Replace("/", ".").Replace(" ", "_").Replace(":", ".") + ".txt";
		...
		// BEGIN GSN EDIT
		if (GUILayout.Button(phoenixContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
		{
			currentView = ReportView.GSN;
		}
		// END GSN EDIT
		...
		if( GUILayout.Button(closeContent ,barStyle, GUILayout.Width(size.x*2) , GUILayout.Height(size.y*2)))
		{
			...
			// BEGIN GSN EDIT
			// TODO: Consider making sure that this doesn't destroy a real blocker
			EventDispatcher.DispatchEvent(new FetchGlobalCompleteEvent());
			// END GSN EDIT
	...
	public void OnGUIDraw()
	{
		...
		if( currentView == ReportView.Info )
			DrawInfo();
		// BEGIN GSN EDIT
		else if (currentView == ReportView.GSN)
			DrawGSN();
		// END GSN EDIT
	...
	IEnumerator readInfo(  )
	{
       ...
       // float startTime = Time.realtimeSinceStartup;
		...
		{
			Debug.LogError( "Trying to load " + url + " and i got " + www.error );
		}
		else 
		{
			buildDate = www.text ;
